...
export class DraggableDirective implements AfterViewInit, OnDestroy {
  ...
  private setForeground() {
    ...
    if (!this.isActivePanelForegroundEnabled()) return;
    const MIN_Z = 9000;
    let stacks = this.elementRef.nativeElement.ownerDocument.querySelectorAll<HTMLElement>(this.stackSelector);
    let topZindex: number = MIN_Z;
    let bottomZindex: number = 99999;
    let currentZindex: number = parseInt(this.elementRef.nativeElement.style.zIndex) || MIN_Z;
    let alreadyTop = true;
    stacks.forEach(elm => {
      let zIndex = parseInt(elm.style.zIndex) || MIN_Z;
      if (elm !== this.elementRef.nativeElement && currentZindex <= zIndex) alreadyTop = false;
    ...
    stacks.forEach(elm => {
      elm.style.zIndex = Math.max(MIN_Z, (parseInt(elm.style.zIndex) - bottomZindex) + MIN_Z) + '';
    });
    this.elementRef.nativeElement.style.zIndex = (topZindex + 1) + '';
  }

  private isActivePanelForegroundEnabled(): boolean {
    try {
      return localStorage.getItem('udonarium.panel.activeForeground.v1') !== '0';
    } catch (_) {
      return true;
    }
  }
