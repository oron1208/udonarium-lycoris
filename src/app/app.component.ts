...
少しだけ、胸が痛むの`;
  ...
  private addDefaultUpdateNoticeNote() {
    ...
    const text = `ユドナリウムリコリス v1.21.0 更新メモ

◆VNステージのログイン管理
・VNモードON時にログインするとフリーズする問題を修正
・部屋に入る際、VNステージを一時的に非表示にして同期完了後に復帰
・後から入室したプレイヤーのVNステージにも立ち絵が同期されるよう修正
・後からVNモードをONにした際も既存の立ち絵が表示されるよう修正

◆VNステージのセリフポップ修正
・他のプレイヤーのVNチャット発言の吹き出しが表示されない問題を修正
・UPDATE_GAME_OBJECTイベント経由で他ピアのチャットメッセージを検知するよう対応

◆パフォーマンス改善
・charactersゲッターにキャッシュを追加（ObjectStoreスキャン頻度を削減）
・VNステージのイベント登録を初期化後1秒遅延（同期中の負荷軽減）
