...
export class CutInListComponent implements OnInit, OnDestroy {
  ...
  set cutInWidth(cutInWidth: number) {
    ...
    if ( this.keepImageAspect ){
      ...
      Logger.debug(' this.keepImageAspect H' + this.selectedCutIn.height);
    }
  }
  ...
  set cutInHeight(cutInHeight: number) {
    ...
    if ( this.keepImageAspect ){
      ...
      Logger.debug(' this.keepImageAspect W' + this.selectedCutIn.width);
    }
  }
    ...
    if ( this.cutInOriginalSize ){
      if (this.isYouTubeCutIn){
        ...
        if ( this.selectedCutIn.width != width){
          ...
          Logger.debug(' setCutInYouTubeSize w:' + width);
        }
      }else{
        const width = this.originalImgWidth();
        if ( this.selectedCutIn.width != width ){
          this.selectedCutIn.width = width;
          Logger.debug(' setCutInOriginalSize w' + width);
        }
      }
    }
    ...
    if ( this.cutInOriginalSize ){
      if (this.isYouTubeCutIn){
        ...
        if ( this.selectedCutIn.height != height){
          ...
          Logger.debug(' setCutInYouTubeSize h:' + height);
        }
      }else{
        const height = this.originalImgHeight();
        if ( this.selectedCutIn.height != height){
          this.selectedCutIn.height = height;
          Logger.debug(' setCutInOriginalSize h' + height);
        }
      }
    }
    ...
    setTimeout(() => {
      if ( this.keepImageAspect ){
        ...
        if ( imageurl.length > 0 ){
          let img = new Image();
          img.src = imageurl;
          Logger.debug('img.height /  img.width ' + img.height + ' ' +  img.width);
          if (this.isYouTubeCutIn){
  ...
  openCutInBgmModal() {
    ...
    this.modalService.open<string>(CutInBgmComponent).then(value => {
      ...
      if ( audio ){
        ...
        Logger.debug('cutInAudioName' + this.cutInAudioName);
      }
    });
  }
  ...
  changeYouTubeInfo() {
    Logger.debug('changeYouTubeInfo');
    if ( !this.selectedCutIn )return;
  ...
  setDefaultControl(isVideo: boolean){
    if ( !this.isEditable ) return 0;
    if ( !this.selectedCutIn ) return 0;
    Logger.debug('setDefaultControl');
/*
...
*/
    if (isVideo){
       Logger.debug('setDefaultControl isVideo');
