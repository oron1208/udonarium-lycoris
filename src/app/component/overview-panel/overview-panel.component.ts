...
export class OverviewPanelComponent implements AfterViewInit, OnDestroy {
  ...
  get isSecretCharacter(): boolean { return this.tabletopObject instanceof GameCharacter && !!this.tabletopObject.secretDetails && !this.gmModeService.isGm; }
  get displayName(): string { return (this.tabletopObject as any).name || ''; }
  ...
  private collectBuffElements(root: DataElement): DataElement[] {
    let result: DataElement[] = [];
    for (let child of root.children as DataElement[]) {
      if (0 < child.children.length) {
        result = result.concat(this.collectBuffElements(child));
      } else if (child.type === 'numberResource') {
        result.push(child);
      }
    }
    return result;
