...
export class PeerCursorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private isGmCursorShareDisabled = this.loadGmCursorShareDisabled();
  private suppressLogoutMessage = false;
  ...
  ngOnInit() {
    if (this.isMine) {
      this.cursor.isCursorShareDisabled = this.isGmCursorShareDisabled;
      this.cursor.update();
    }
...
//            Logger.debug( '★★HEART_BEAT★★ ' + event.sendFrom + ' > ' + this.cursor.peerId );
    ...
    EventSystem.register(this)
      .on('GM_CURSOR_SHARE_DISABLED_CHANGED', event => {
        this.isGmCursorShareDisabled = !!event.data.disabled;
        if (this.isMine) {
          this.cursor.isCursorShareDisabled = this.isGmCursorShareDisabled;
          this.cursor.update();
        }
      })
      .on('TABLE_VIEW_SOFT_RESET_START', event => {
        this.suppressLogoutMessage = true;
      });
  }
  ...
  private logoutMessage(){
    if (this.suppressLogoutMessage) return;
    const chatTabList = ObjectStore.instance.get<ChatTabList>('ChatTabList');
    const sysTab = chatTabList.systemMessageTab;
    if(sysTab){
...
//            Logger.debug( 'peerlength:' + peerlength + 'this.indexCounter' + this.indexCounter + ' id:' + id);
  ...
  private calcLocalCoordinate(x: number, y: number, target: HTMLElement) {
    ...
    if (this.isGmCursorShareDisabled) return;
