...
export class TerrainComponent implements OnInit, OnDestroy, AfterViewInit{
  ...
  get hasUrlScreen(): boolean { return this.terrain.hasUrlScreen; }
  get urlScreenUrl(): string { return this.terrain.urlScreenUrl || ''; }
  get urlScreenFace(): string { return this.terrain.urlScreenFace || 'top'; }
  set urlScreenFace(face: string) { this.terrain.urlScreenFace = face; }
  get urlScreenEmbedUrl(): string { return this.toEmbeddableUrl(this.urlScreenUrl); }
  ...
  onContextMenu(e: Event) {
    ...
    this.contextMenuService.open(menuPosition, [
      ...
      ContextMenuSeparator,
      { name: this.hasUrlScreen ? 'URLスクリーンを変更' : 'URLスクリーンを設定', action: () => { this.setUrlScreen(); } },
      { name: 'URLスクリーン面', action: null, subActions: [
        { name: `${ this.urlScreenFace === 'top' ? '◉' : '○' } 上面`, action: () => { this.urlScreenFace = 'top'; } },
        { name: `${ this.urlScreenFace === 'north' ? '◉' : '○' } 北面`, action: () => { this.urlScreenFace = 'north'; } },
        { name: `${ this.urlScreenFace === 'south' ? '◉' : '○' } 南面`, action: () => { this.urlScreenFace = 'south'; } },
        { name: `${ this.urlScreenFace === 'east' ? '◉' : '○' } 東面`, action: () => { this.urlScreenFace = 'east'; } },
        { name: `${ this.urlScreenFace === 'west' ? '◉' : '○' } 西面`, action: () => { this.urlScreenFace = 'west'; } },
      ]},
      ...(this.hasUrlScreen ? [{ name: 'URLスクリーンを消去', action: () => { this.terrain.urlScreenUrl = ''; SoundEffect.play(PresetSound.sweep); } }] : []),
      ContextMenuSeparator,
  ...
  private setUrlScreen() {
    let url = window.prompt('地形に表示するURLを入力してください（YouTube URL / https URL）', this.urlScreenUrl);
    if (url == null) return;
    url = url.trim();
    if (url.length && !/^https?:\/\//i.test(url)) {
      window.alert('URLは http:// または https:// から始めてください。');
      return;
    }
    this.terrain.urlScreenUrl = url;
    SoundEffect.play(PresetSound.blockPut);
  }

  isUrlScreenFace(face: string): boolean {
    return this.hasUrlScreen && this.urlScreenFace === face;
  }
