...
| バージョン | 日付 | 主な変更 |
|---|---|---|
| v1.57.0 | 2026-07-09 | 初期メディア読み込み優先度制御、コマ画像高品質縮小表示、多段階リサイズ改善 |
| v1.56.0 | 2026-07-09 | 重いファイル処理のWorker化、画像処理中のUI停止軽減 |
| v1.55.0 | 2026-07-09 | ファイル名表示修正、アップロード高速化、ZIP保存画像漏れ防止 |
| v1.54.0 | 2026-07-05 | サーバー配信化強化、平面モード改善、ホットバー編集、部位管理ホバーパネル |
...
## ⚠️ docs-site デプロイ時の注意

### --delete を使わないこと

`docs-site`のデプロイには`--delete`オプションを**絶対に使わない**こと。

VPSの`/var/www/docs-site/`には、ローカルの`docs-site/`に含まれないファイルが存在する場合がある：
- `library/`（ミュージックライブラリページ）
- その他VPS上で直接編集されたファイル

### デプロイスクリプト

`scripts/deploy-docs-site.sh`を使用すること：

```bash
bash scripts/deploy-docs-site.sh
```

このスクリプトは`--delete`なしでrsyncする。VPS側のファイルを削除せず、ローカルの変更を上書き送信するだけ。

### どうしても --delete が必要な場合

VPSのファイルを完全にローカルと同期したい場合のみ、手動で`--delete`を指定する。ただし、その前にVPS側にしかないファイルがないか必ず確認すること：

```bash
ssh -i VPSサーバ….pem root@160.251.182.194 'ls -R /var/www/docs-site/'
```
